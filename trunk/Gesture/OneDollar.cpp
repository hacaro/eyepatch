...
int Recognizer::DeleteUserTemplates() {
	m_templates.erase(m_templates.begin() + NumTemplates, m_templates.end());
	return NumTemplates;
}
...
vector<OneDollarPoint> ScaleToSquare(vector<OneDollarPoint> points, double size) {
	...
	if (sideratio < 0.3) {	// we consider this a 1D gesture and scale it uniformly
		scalex = size / longside;
		scaley = size / longside;
	} else {	// this is a 2D gesture so we will scale it non-uniformly
...
double Distance(OneDollarPoint p1, OneDollarPoint p2) {
	return _hypot(p2.m_x-p1.m_x, p2.m_y-p1.m_y);
}
