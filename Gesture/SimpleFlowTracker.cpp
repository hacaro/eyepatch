...
//		cvCircle(outputFrame, p, 3, CV_RGB(0,255,0)); 
//		cvLine(outputFrame, p, q, CV_RGB(0,255,0), 1, CV_AA);
...
//	cvLine(outputFrame, cvPoint(outputFrame->width/2, outputFrame->height/2),
//		cvPoint(outputFrame->width/2+30*mvecx, outputFrame->height/2+30*mvecy), CV_RGB(255,255,255), 2, CV_AA);
	...
	DrawTrack(outputFrame, scaledpts, CV_RGB(100,255,100), 3, GESTURE_SQUARE_SIZE);

	// don't let the current trajectory grow longer than the max allowed trajectory length
	if (trajectory.size() > GESTURE_MAX_TRAJECTORY_LENGTH) trajectory.pop_front();
}
...
void SimpleFlowTracker::ClearCurrentTrajectory() { // this function is called after a successful match when running live
	trajectory.clear();
}
